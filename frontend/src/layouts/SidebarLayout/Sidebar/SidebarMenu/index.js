...
import menuItems from './student_items';
import instuctorItems from './instructor_items';
...
const reduceChildRoutes = ({
  ...
  if (item.items) {
    const partialMatch = item.link ? matchPath(path,{
      path: item.link,
      exact: true,
      end: false
    }, path) : false;
    ...
    ev.push(
      <SidebarMenuItem
        ...
        open={partialMatch}
        name={item.name}
        start_icon={item.start_icon}
        end_icon={item.end_icon}
        link={item.link}
        badge={item.badge}
      >
